...
export default function HostDetailsPage() {
  ...
  const [data, setData] = useState<{ ip: string; mac?: string; services: any[]; nmap_error?: string } | null>(null);
  const [whois, setWhois] = useState<any | null>(null);
  const [shodan, setShodan] = useState<any | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(e?.message || "Failed to load host details");
      } finally {
        setLoading(false);
      }
    };
  ...
  const services = useMemo(() => (data?.services || []).sort((a: any, b: any) => a.port - b.port), [data]);
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex gap-2 mb-4">
        {['overview','services','intel'].map(t => (
          <button key={t} onClick={()=>setActiveTab(t as any)} className={`px-3 py-2 rounded ${activeTab===t? 'bg-cyan-700 text-white':'bg-gray-800 text-gray-300'}`}>{t.toUpperCase()}</button>
        ))}
      </div>
